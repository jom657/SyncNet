...
class An(LoadFile):  
    ...
    def get_trunk(self, node_name, vlan_list):
        ...
        if (vlan_list): # if vlan_list has data
            ...
            vlan_dump_list = self.to_list(vlan_dump)
            ag1, ag2 = '-', '-'
            ...
            for vlan in vlan_dump_list:
                try:
                    if not ag_homing_list:
                        ...
                        ag_homing_list = self.get_ag_from(f"interface {trunk}.{vlan}","mpls")
                        # print(f"<{node_name} and {ag_homing_list}>")
                        # if len(ag_homing_list):
            ...
            trunk_array += [area,node_name,self.equipment_name,trunk,om,sip,hsi,ipoe,h248,rtp,ag1,ag2,auth_ip,unauth_ip]
